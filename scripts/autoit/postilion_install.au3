...
; Screen 6 adds 5 Edit fields for database file configuration:
;   DB Name, Data Device, Data File Path, Log Device, Log File Path
; These are the LAST 5 Edit controls on this screen.
Local $dbNameInst6 = $editCountS6 - 4
Local $dataDevInst = $editCountS6 - 3
Local $dataPathInst = $editCountS6 - 2
Local $logDevInst = $editCountS6 - 1
Local $logPathInst = $editCountS6
...
; Count total Edit controls — Screen 7 adds 1 Edit for hostname (the last one)
Local $editCountS7 = _CountEdits()
_Log("[Screen 7] Total Edit controls: " & $editCountS7 & " — hostname is at INSTANCE:" & $editCountS7)

_Log("[Screen 7] Setting Services Server hostname (INSTANCE:" & $editCountS7 & "): " & $svcHostname)
ControlSetText($mainWindow, "", "[CLASS:Edit; INSTANCE:" & $editCountS7 & "]", $svcHostname)
Sleep(500)

; Verify write
Local $verifyHost = ControlGetText($mainWindow, "", "[CLASS:Edit; INSTANCE:" & $editCountS7 & "]")
...
; Screen 8 adds 3 fields (Domain, Username, Password) at the end
; So they should be at maxEdit-2, maxEdit-1, maxEdit
Local $domainInstance = $maxEdit - 2
Local $usernameInstance = $maxEdit - 1
Local $passwordInstance = $maxEdit
